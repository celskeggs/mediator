...
func (d DefinedType) addContext(dt *DefinedTree) DefinedType {
	...
	for i, orig := range origInits {
		dPtr.Inits[i].definingStruct, dPtr.Inits[i].longName = dt.ResolveField(d.TypePath, orig.ShortName)
...
func (t *DefinedTree) ResolveField(typePath string, shortName string) (definingStruct string, longName string) {
	...
	for _, field := range defType.Fields {
		if field.Name == shortName {
			return defType.StructName(), field.LongName()
		}
	}
